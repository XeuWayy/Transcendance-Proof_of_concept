...
const suzanneLight = new THREE.PointLight('white', 5)
suzanneLight.position.set(0, 5, 7)
scene.add(suzanneLight)
...
const lodDistance = [4, 8, 16]
lod.position.set(0, 5, 5)
let i = 0
const suzanne = gltfLoader.load('./gltf/suzanne_lod/suzanne_lod.glb', (suzanne) => {
    suzanne.scene.position.set(0, 2, 5)
    suzanneModel = suzanne.scene
    suzanneModel.traverse((child) => {
        if (child.isMesh) {
            console.log(child)
            lod.addLevel(child, lodDistance[i])
            i++
        }
    });
})
